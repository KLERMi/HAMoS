...
def get_session():
    ...
    # Define each session’s date and its 2 PM deadline
    sessions = [
    ...
    for session_date, col_name in sessions:
        ...
        if now.date() < session_date:
            ...
            # on the session day, before 2 PM
            return col_name

    # past all session deadlines
    return None
    ...
    with st.form("find_form"):
        ...
        if st.form_submit_button("Find Record"):
            ...
            match  = next((r for r in recs 
                           if str(r.get("phone","")) == lookup 
                           or str(r.get("tag",""))   == lookup), None)
            if not match:
