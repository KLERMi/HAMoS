...
try:
    ...
    # --- Default selection ---
    if ('selected_name' not in st.session_state or st.session_state['selected_name'] is None) and not display_df.empty:
    ...
    if match_rows.empty:
        if not display_df.empty:
            ...
            st.warning("No valid attendees to select from.")
            st.stop()
